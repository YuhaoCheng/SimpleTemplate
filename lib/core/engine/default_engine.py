...
class DefaultTrainer(AbstractTrainer):
    ...
    def data_parallel(self, model):
        ...
        # print('The gpus are:', gpus)
        # import ipdb; ipdb.set_trace()
        # model_parallel = torch.nn.DataParallel(model, device_ids=gpus).cuda()
        model_parallel = torch.nn.parallel.DistributedDataParallel(model.cuda())
    ...
    def before_step(self, current_step):
        # self.logger.info('-0_0- ==>|{}| Start Train the {}/{} step'.format(self._get_time(), current_step, self.config.TRAIN.max_steps))
        pass
    ...
    def after_step(self, current_step):
        ...
        if (current_step % self.eval_step != 0) or current_step == 0:
            # print(f'not eval whole at {current_step}')
        ...
        #         self.save(current_step, best=True)
        #     elif current_step % self.save_step == 0:
        #         # save the checkpoint
        #         self.save(current_step)
        #         self.logger.info('LOL==>the accuracy is not imporved in epcoh{} but save'.format(current_step))
        #     else:
        #         pass
    ...
    def save(self, current_epoch, best=False):
        ...
        '''
        if best:
            engine_save_checkpoint(self.config, self.kwargs['config_name'], self.saved_model, current_epoch, self.saved_loss, self.saved_optimizer, self.logger, self.kwargs['time_stamp'], self.accuarcy, flag='best', verbose=(self.kwargs['model_type'] + '#' + self.verbose))
            self.result_path = engine_save_model(self.config, self.kwargs['config_name'], self.saved_model, self.logger, self.kwargs['time_stamp'], self.accuarcy, verbose=(self.kwargs['model_type'] + '#' + self.verbose))
        else:
            engine_save_checkpoint(self.config, self.kwargs['config_name'], self.saved_model, current_epoch, self.saved_loss, self.saved_optimizer, self.logger, self.kwargs['time_stamp'], self.accuarcy, verbose=(self.kwargs['model_type'] + '#' + self.verbose))

        # del self.saved_model
        # del self.saved_loss
        # del self.saved_optimizer
